...
Для изменения администратором конфигурации OTP-кода необходимо отправить запрос методом `POST` по URL `http://localhost:8080/api/admin/otp-config`
...
Для удаления администратором пользователя и привязанных к нему OTP-кодов необходимо отправить запрос методом `POST` по URL `http://localhost:8080/api/admin/delete-user`
